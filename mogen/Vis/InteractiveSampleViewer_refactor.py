...
class InteractiveSampleViewer:
    def __init__(self, *, i_world=0, i_sample=0, directory=None,
                 exp_par, gd_par, par,
                 show_buttons=True):
        ...
        self.directory = get_sample_dir(directory)

        if self.directory is None:
            self.n_worlds = 0
        else:
            self.n_worlds = ld_sql.get_n_rows(file=self.directory + WORLD_DB)
        ...
        self.world_viewer = WorldViewer(par=self.par.world, ax=self.ax, i_world=self.i_world,
                                        directory=self.directory)
        self.sphere_path = SpherePath(i_sample=self.i_sample_global, ax=self.ax, directory=self.directory,
    ...
    def init_buttons(self):
        if self.show_buttons:
            self.b_predict = plt2.create_button(axes=[0.8, 0.1, 0.08, 0.06], name='Predict',
                                                listener_fun=self.plot_prediction)

            self.b_initial_guess = plt2.create_button(axes=[0.8, 0.2, 0.08, 0.06], name='Initial Guess',
                                                      listener_fun=self.initial_guess_plot)

            self.b_initial_guess_random = plt2.create_button(axes=[0.8, 0.3, 0.08, 0.06], name='Initial Guess Random',
                                                             listener_fun=self.initial_guess_plot)

            self.b_optimize = plt2.create_button(axes=[0.8, 0.4, 0.08, 0.06], name='Optimize',
