...
x_path, objective = ld_sql.get_values_sql(columns=[PATH_Q, 'objective', 'objective_pred'],
                                          i_worlds=i_world, directory=directory, values_only=True)
img_dir = d.PROJECT_DATA_IMAGES + 'World_Statistics/' + directory
n_samples = x_path.shape[0]
objective = objective.reshape(n_worlds, d.n_samples_per_world)

x_path_inner = path.x2x_inner(x=x_path, n_dof=n_dim, n_samples=n_samples)
...
hist_total = np.histogram(objective, bins=bins, range=hist_range)[0] / n_worlds
...
ax.set_title(
    'GridWorld: {}, Coverage: {}, Mean Objective: {} - most expensive'.format(iw, obst_coverage[iw], objective_pw[iw]))
plt2.plot_img_patch_w_outlines(limits=world_size, n_voxels=n_voxels, img=obstacle_img[iw], ax=ax, n_dim=n_dim)
save_fig(img_dir + 'world__most_expensive', fig=fig, save=save_img)

# Easiest (Cheapest) GridWorld
iw = np.argmin(objective_pw)
fig, ax = plt2.new_world_fig(limits=world_size)
ax.set_title('GridWorld: {}, Coverage: {}, Mean Objective: {} - cheapest'.format(iw, obst_coverage[iw], objective_pw[iw]))
plt2.plot_img_patch_w_outlines(limits=world_size, n_voxels=n_voxels, img=obstacle_img[iw], ax=ax, n_dim=n_dim)
...
fig, ax = plt2.new_world_fig(limits=world_size)
ax.set_title('GridWorld: {}, Coverage: {}, Mean Objective: {} - densest'.format(iw, obst_coverage[iw], objective_pw[iw]))
plt2.plot_img_patch_w_outlines(limits=world_size, n_voxels=n_voxels, img=obstacle_img[iw], ax=ax, n_dim=n_dim)
...
fig, ax = plt2.new_world_fig(limits=world_size)
ax.set_title(
    'GridWorld: {}, Coverage: {}, Mean Objective: {} - second densest'.format(iw, obst_coverage[iw], objective_pw[iw]))
plt2.plot_img_patch_w_outlines(limits=world_size, n_voxels=n_voxels, img=obstacle_img[iw], ax=ax, n_dim=n_dim)
...
# PATH LENGTH
path_length = path.path_length(x_path, n_dim=n_dim, n_samples=n_samples)
direct_path_length = path.linear_distance(x=x_path, n_samples=n_samples)
...
ax.set_ylabel('Number of Paths')
ax.hist(relative_path_length, bins=50, range=[1, 2])
...
path_length_pw = path_length.reshape(n_worlds, d.n_samples_per_world).mean(axis=1)
direct_path_length_pw = direct_path_length.reshape(n_worlds, d.n_samples_per_world).mean(axis=1)
relative_path_length_pw = relative_path_length.reshape(n_worlds, d.n_samples_per_world).mean(axis=1)
...
if plot_prediction:
    ...
    loss = c_loss.euclidean_loss_np(y_true=x_path_inner, y_pred=x_pred_inner, n_dim=n_dim, batch_size=n_samples,
                                    world_size=1)
    loss_pw = loss.reshape(n_worlds, d.n_samples_per_world).mean(axis=1)

    fig, ax = new_fig()
    ax.set_xlabel('Loss of Neural Network, mean={}'.format(loss.mean()))
    ax.hist(loss, bins=50, range=[0, 10])
    save_fig(d.PROJECT_DATA_IMAGES + 'path_stat__hist__network_loss', fig=fig, save=save_img)
    ...
    ax.set_xlabel('Relative Path Length')
    ax.set_ylabel('Network Loss')
    img = ax.hist2d(relative_path_length, loss, norm=mpl.colors.LogNorm(), bins=100, range=[[1, 3], [0, 40]])[-1]
    fig.colorbar(img, ax=ax)
    ...
    for i in range(1, 11):
        ...
        plt2.plot_x_path(x=x_pred[idx_sorted_loss[-i]], n_dim=n_dim, ax=ax, marker='o')
