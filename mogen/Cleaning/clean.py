...
def reset_sample_i32(file):
    ...
    iw_all = sql2.get_values_sql(file=file, table=data.T_PATHS(), rows=-1, columns=[data.T_PATHS.C_WORLD_I()])
    ...
    sql2.set_values_sql(file=file, table=data.T_PATHS(), values=(i_s,), columns=data.T_PATHS.C_SAMPLE_I())
...
def remove_infeasible(file):
    ...
    if not np.all(feasible):
        sql2.delete_rows(file=file, table=data.T_PATHS, rows=~f)
...
def set_dtypes(file):
    ...
    columns_paths_new = [data.T_PATHS.C_WORLD_I(), data.T_PATHS.C_SAMPLE_I(),
                         data.T_PATHS.C_Q_F(), data.T_PATHS.C_OBJECTIVE_F(), data.T_PATHS.C_FEASIBLE_I()]
